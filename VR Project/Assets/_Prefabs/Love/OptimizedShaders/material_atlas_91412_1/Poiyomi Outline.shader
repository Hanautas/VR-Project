Shader "Hidden/Locked/.poiyomi/Poiyomi 8.0/Poiyomi Outline/51977127c8843014db6a565a6f0462ef"
{
	Properties
	{
 ...
 #define _LIGHTINGMODE_FLAT 
			...
			float4 frag(v2f i, uint facing : SV_IsFrontFace) : SV_Target
			{
				...
				if (float(0))
				{
					mainUV = sharpSample(float4(0.0001220703,0.0002441406,8192,4096), mainUV);
				}
 ...
 #define _LIGHTINGMODE_FLAT 
			...
			void applyOutlineColor(inout PoiFragData poiFragData, in PoiMesh poiMesh, in PoiLight poiLight, in PoiMods poiMods)
			{
				...
				float4 col = POI2D_SAMPLER_PAN(_OutlineTexture, _MainTex, poiUV(poiMesh.uv[float(0)], float4(1,1,0,0)), float4(0,0,0,0)) * float4(poiThemeColor(poiMods, float4(0,0,0,1).rgb, float(0)), float4(0,0,0,1).a);
